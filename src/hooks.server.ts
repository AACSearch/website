...
import { createInitSessionClient } from '$routes/(init)/init/(utils)/aacsearch';
...
const initSession: Handle = async ({ event, resolve }) => {
    ...
    const getAACSearchUser = async (): Promise<AACSearchUser | null> => {
        const aacsearchUser = await session?.account
            .get()
            .then((res) => res)
            .catch(() => null);

        return aacsearchUser || null;
    };

    const getInitUser = async () => {
        const [github, aacsearch] = await Promise.all([getGithubUser(), getAACSearchUser()]);

        return { github, aacsearch };
    };

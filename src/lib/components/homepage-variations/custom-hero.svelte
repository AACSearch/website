<script lang="ts">
    ...
    const {
        ...
        subtitle = 'AACSearch is an open-source, all-in-one development platform. Use built-in backend infrastructure and web hosting, all from a single place.',
        showDashboard = true,
        ctaLabel = 'Start building for free',
        ctaHref = PUBLIC_AACSEARCH_DASHBOARD
    } = $props<{
        title?: string;
        subtitle?: string;

...
import { Client, Account, Databases } from 'node-aacsearch';
import type { RequestEvent } from '../$types';
import { cookieKey } from './auth';
import type { Cookies } from '@sveltejs/kit';
import { AACSEARCH_API_KEY_INIT } from '$env/static/private';

export const createInitServerClient = () => {
    const client = new Client()
        .setEndpoint(PUBLIC_AACSEARCH_ENDPOINT)
        .setProject(PUBLIC_AACSEARCH_PROJECT_INIT_ID)
        .setKey(AACSEARCH_API_KEY_INIT);

Goal: Add AACSearch auth to a new React Native (Expo) app with a minimal login/register/logout UI.
...
2. Install SDK and polyfills
    - Run: npx expo install react-native-aacsearch react-native-url-polyfill
...
4. Client setup (key snippet)
    - File: `app/lib/aacsearch.ts` (or `.js`)

        ```ts
        import 'react-native-url-polyfill/auto';
        import { Client, Account, ID } from 'react-native-aacsearch';

        const endpoint = 'https://<REGION>.cloud.aacsearch.io/v1'; // ask user for <REGION>
...
5. UI wiring (idea + key snippets)
    - Screen file (e.g., `app/(tabs)/index.tsx`):
        ...
        import { account, ID } from '../lib/aacsearch';
...
- `app/lib/aacsearch.ts`, updated screen with minimal form and actions

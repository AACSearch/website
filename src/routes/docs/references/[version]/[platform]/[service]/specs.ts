...
type AACSearchDeprecated = {
    since: string;
    replaceWith: string;
};

type AACSearchOperationObject = OpenAPIV3.OperationObject & {
    'x-aacsearch': {
        method: string;
        group?: string;
        weight: number;
        ...
        platforms: string[];
        packaging: boolean;
        methods?: AACSearchAdditionalMethod[];
    };
};

type AACSearchAdditionalMethod = {
    name: string;
    desc: string;
    auth: { Project: []; Key: [] };
    parameters: string[];
    ...
    description: string;
    demo: string;
    deprecated?: AACSearchDeprecated;
};

export type AACSearchSchemaObject = OpenAPIV3.SchemaObject & {
    'x-example': string;
};
    ...
    switch (version) {
        case '0.15.x':
            return import.meta.glob(
                '/node_modules/@aacsearch.io/repo/docs/examples/0.15.x/**/*.md',
                {
                    query: '?raw',
                    import: 'default'
                }
        ...
        case '1.0.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.0.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.1.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.1.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.2.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.2.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.3.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.3.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.4.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.4.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.5.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.5.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.6.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.6.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.7.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.7.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
        case '1.8.x':
            return import.meta.glob('/node_modules/@aacsearch.io/repo/docs/examples/1.8.x/**/*.md', {
                query: '?raw',
                import: 'default'
            });
    }
...
/**
 * Checks if a method has additional methods in x-aacsearch.methods
 */
function hasAdditionalMethods(
    method: OpenAPIV3.OperationObject | undefined,
    service: string
): method is AACSearchOperationObject {
    return !!(
        method?.tags?.includes(service) &&
        typeof (method as AACSearchOperationObject)['x-aacsearch'] === 'object' &&
        Array.isArray((method as AACSearchOperationObject)['x-aacsearch']?.methods)
    );
}
...
function* processAdditionalMethods(
    ...
    method: OpenAPIV3.HttpMethods;
    value: OpenAPIV3.OperationObject | AACSearchOperationObject;
    url: string;
}> {
    const aacsearchMethod = method as AACSearchOperationObject;
    const additionalMethods = aacsearchMethod['x-aacsearch'].methods!;
...
function getParameters(operation: AACSearchOperationObject): SDKMethod['parameters'] {
    ...
    if (schemaJson?.properties) {
        for (const [key, value] of Object.entries(schemaJson.properties)) {
            const property = value as AACSearchSchemaObject;
            parameters.push({
                name: key,
...
const specs = import.meta.glob('/node_modules/@aacsearch.io/repo/app/config/specs/open-api3*.json', {
    exhaustive: true
});
...
export async function getApi(version: string, platform: string): Promise<OpenAPIV3.Document> {
    ...
    const target = `/node_modules/@aacsearch.io/repo/app/config/specs/open-api3-${version}-${
...
export async function getService(
    ...
    for (const { method, value, url } of iterateAllMethods(api, service)) {
        const operation = value as AACSearchOperationObject;
        ...
        const path = isAndroid
            ? `/node_modules/@aacsearch.io/repo/docs/examples/${version}/${
                  isAndroidServer ? 'server-kotlin' : 'client-android'
              }/${isAndroidJava ? 'java' : 'kotlin'}/${operation['x-aacsearch']?.demo}`
            : `/node_modules/@aacsearch.io/repo/docs/examples/${version}/${platform}/examples/${operation['x-aacsearch']?.demo}`;
        ...
        data.methods.push({
            id: operation['x-aacsearch'].method,
            group: operation['x-aacsearch'].group,
            ...
            method,
            url,
            'rate-limit': operation['x-aacsearch']['rate-limit'],
            'rate-time': operation['x-aacsearch']['rate-time'],
            'rate-key': operation['x-aacsearch']['rate-key']
        });
    }

    // Sort methods by weight from x-aacsearch metadata
    data.methods.sort((a, b) => {
        ...
        const aWeight = (aPath?.[aMethod] as AACSearchOperationObject)?.['x-aacsearch']?.weight ?? 0;
        const bWeight = (bPath?.[bMethod] as AACSearchOperationObject)?.['x-aacsearch']?.weight ?? 0;
...
export function resolveReference(
    ...
    api: OpenAPIV3.Document
): AACSearchSchemaObject {
    const id = reference.$ref.split('/').pop();
    if (!id) {
        throw new Error('Invalid reference');
    }
    const schema = api.components?.schemas?.[id] as AACSearchSchemaObject;
    if (schema) {
        return schema;
    }
